        ...
        try:
            with session.begin() as conn:
                for dict_row in conn.execute(select_bsc_info_stmt).all():
                    ...
                    pvdr_site_cd = tn_data_bsc_info.pvdr_site_cd.lower() # 제공처사이트코드
                    link_clct_mthd_dtl_cd = tn_data_bsc_info.link_clct_mthd_dtl_cd.lower() # 연계수집방식 상세코드
                    ...
                    # 기상청_단기예보, 5분_소통정보, 센서측정정보, 대기오염정보_측정소별_실시간_측정정보_조회, 실시간_측정정보_조회, 새올행정주민요약DB, 강우량,통계청 로그 존재 확인
                    if (dtst_cd in {"data852", "data4", "data799", "data31", 'data855', "data793", "data795", "data792", "data794","data1052"} or (link_clct_mthd_dtl_cd in ("open_api","on_file","rdb","tcp") and pvdr_site_cd in {"ps00010"})) :
                        th_data_clct_mastr_log = CommonUtil.get_exist_log(conn, data_crtr_pnttm, dtst_cd)

                    # 기상청_단기예보, 5분_소통정보, 센서측정정보, 대기오염정보_측정소별_실시간_측정정보_조회, 실시간_측정정보_조회, 새올행정주민요약DB, 강우량,통계청 의 경우 로그 존재하지 않을 때
                    if ((dtst_cd in {"data852", 'data4', 'data799', 'data31', 'data855', "data793", "data795", "data792", "data794","data1052"} or (link_clct_mthd_dtl_cd in ("open_api","on_file","rdb","tcp") and pvdr_site_cd in {"ps00010"})) and th_data_clct_mastr_log == None) \
                         or (dtst_cd not in {"data852", 'data4', 'data799', 'data31', 'data855', "data793", "data795", "data792", "data794","data1052"} and (link_clct_mthd_dtl_cd in ("open_api","on_file","rdb","tcp") and pvdr_site_cd not in {"ps00010"})) :
                        # or (dtst_cd not in {"data852", 'data4', 'data799', 'data31', 'data855', "data793", "data795", "data792", "data794","data1052"} and (link_clct_mthd_dtl_cd in ("open_api","on_file") and (pvdr_site_cd not in {"ps00010"}))):
                    ...
                    # 통계청일 경우 기존 로그가 있어도 21일에 데이터 재수집 (update) 
                    if (link_clct_mthd_dtl_cd in ("open_api","on_file","rdb","tcp") and pvdr_site_cd in {"ps00010"}) or ((link_clct_mthd_dtl_cd in ("open_api","on_file","rdb","tcp") and pvdr_site_cd in {"ps00010"})) and ((th_data_clct_mastr_log.step_se_cd == CONST.STEP_FILE_INSD_SEND and th_data_clct_mastr_log.stts_cd == CONST.STTS_COMP)\
                        or th_data_clct_mastr_log.step_se_cd == CONST.STEP_FILE_STRGE_SEND or th_data_clct_mastr_log.step_se_cd == CONST.STEP_DW_LDADNG):
                        logging.info(f"insert_collect_data_info ::: 1차 수집 로그 존재 확인, 데이터 업데이트성 성공로그 및 실패로그 재수집 실행")
                        th_data_clct_mastr_log.clct_ymd = data_interval_end.strftime("%Y%m%d")
                        th_data_clct_mastr_log.step_se_cd = CONST.STEP_CNTN
                        th_data_clct_mastr_log.stts_cd = CONST.STTS_WORK
                        th_data_clct_mastr_log.stts_msg = CONST.MSG_CNTN_WORK
                    ...
                    # 기상청_단기예보, 5분_소통정보, 센서측정정보, 대기오염정보_측정소별_실시간_측정정보_조회, 실시간_측정정보_조회, 새올행정주민요약DB, 강우량, 통계청 - th_data_clct_stts_hstry_log 입력위한 th_data_clct_mastr_log 설정

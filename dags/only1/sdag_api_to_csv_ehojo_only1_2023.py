...
@dag(
    dag_id="sdag_api_to_csv_ehojo_only1_2023",
    schedule="@daily",
...
def sdag_api_to_csv_ehojo_only1():
    ...
    def insert_collect_data_info(**kwargs):
        ...
        data_interval_start = datetime(2023, 1, 1, tz="Asia/Seoul")  # 2023년으로 고정
        # data_interval_end = kwargs['data_interval_end'].in_timezone("Asia/Seoul")  # 실제 실행하는 날짜를 KST 로 설정
        data_interval_end = datetime(2023, 12, 31, tz="Asia/Seoul")  # 2023년으로 고정
        collect_data_list = []
        try:
            with session.begin() as conn:
                for dict_row in conn.execute(select_bsc_info_stmt).all():
                    ...
                    file_name = tn_data_bsc_info.dtst_nm.replace(" ", "_") + "_2023"

...
@dag(
    dag_id="sdag_api_dw_month_data_2nd",
    schedule="@monthly",
...
def api_dw_month_data_2nd():
    ...
    def collect_data_info(**kwargs): # 수집 데이터 정보 조회
        ...
        select_bsc_info_stmt = '''
                            ...
                            and dtst_cd = 'data921' -- 공공데이터포털 OpenApi
                            order by sn
                            '''
    ...
    def call_url_process(collect_data_list):
            ...
            try:
                ...
                while repeat_num <= params_len:
                    ...
                    while page_no <= total_page:
                        ...
                        # return_url = f"{base_url}{CallUrlUtil.set_url(dtst_cd, params_dict, repeat_num, page_no)}" 
        ...
        def encrypt_zip_file(collect_data_list, file_path, **kwargs):
            ...
            try:
                with session.begin() as conn:
                    ...
                    pvdr_site_nm = tn_data_bsc_info.pvdr_site_nm  #사이트 한글명?
                    link_file_extn = tn_data_bsc_info.link_file_extn
                    pvdr_sou_data_pvsn_stle = tn_data_bsc_info.pvdr_sou_data_pvsn_stle # 제공처 원천 데이터 제공 형태
        ...
        file_path >> call_url(collect_data_list, file_path) >> encrypt_zip_file(collect_data_list, file_path) >> put_file_sftp (collect_data_list, encrypt_file_path)

    collect_data_list = collect_data_info()
    call_url_process.expand(collect_data_list = collect_data_list)

dag_object = api_dw_month_data_2nd()

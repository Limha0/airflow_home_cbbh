...
@dag(
    dag_id="sdag_api_to_csv_year_only1_2026",
    schedule="5 0 20 1 *",
...
def api_to_csv_year():
    ...
    def insert_collect_data_info(**kwargs):
        ...
        data_interval_start = datetime(2026, 1, 1, tz="Asia/Seoul")  # 2026년으로 고정
        data_interval_end = datetime(2026, 12, 31, tz="Asia/Seoul")  # 2026년으로 고정
        collect_data_list = []
        try:
            with session.begin() as conn:
                for dict_row in conn.execute(select_bsc_info_stmt).all():
                    ...
                    file_name = tn_data_bsc_info.dtst_nm.replace(" ", "_") + "_2026"

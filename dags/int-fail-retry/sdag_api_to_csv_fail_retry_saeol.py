...
@dag(
    dag_id="sdag_api_to_csv_fail_retry_saeol",
    schedule="30,45 1 * * *",
    start_date=datetime(2023, 9, 16, tz="Asia/Seoul"),  # UI 에 KST 시간으로 표출하기 위한 tz 설정
    catchup=False,
    # render Jinja template as native Python object
    render_template_as_native_obj=True,
    tags=["api_to_csv_retry", "day", "int"],
)
def api_to_csv_fail_retry_saeol():
    ...
    def select_collect_data_fail_info(**kwargs):
        ...
        select_log_info_stmt = f'''
                            ...
                            WHERE 1=1
                                ...
                                AND LOWER(a.dtst_cd) IN ('data677', 'data762', 'data763') -- 새올_민원통계, 직원정보, 부서정보
                ...
                while repeat_num <= params_len:
                    ...
                    while page_no <= total_page:
                        ...
                        if res_code == "Y":
                            ...
                            # 원천 데이터 저장
                            if link_file_crt_yn == "y":  # 직원정보, 부서정보 원천 파일 생성 안 함
...
dag_object = api_to_csv_fail_retry_saeol()

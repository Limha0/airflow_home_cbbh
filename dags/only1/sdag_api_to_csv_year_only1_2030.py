...
@dag(
    dag_id="sdag_api_to_csv_year_only1_2030",
    schedule="5 0 20 1 *",
...
def api_to_csv_year():
    ...
    def insert_collect_data_info(**kwargs):
        ...
        data_interval_start = datetime(2030, 1, 1, tz="Asia/Seoul")  # 2030년으로 고정
        data_interval_end = datetime(2030, 12, 31, tz="Asia/Seoul")  # 2030년으로 고정
        collect_data_list = []
        try:
            with session.begin() as conn:
                for dict_row in conn.execute(select_bsc_info_stmt).all():
                    ...
                    file_name = tn_data_bsc_info.dtst_nm.replace(" ", "_") + "_2030"
    ...
    def call_url_process(collect_data_list):
        ...
        def call_url(collect_data_list, file_path, **kwargs):
            ...
            # 세입예산결산내역, 세출예산결산내역
            year = "2030"
            # year_list = [2030,2027,2028,2029,2030]
            params_dict = {}

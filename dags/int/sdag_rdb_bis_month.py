...
def sdag_rdb_bis():
    ...
    def db_to_csv_process(collect_data_list):
        ...
        def create_csv_file(collect_data_list, db_connection_url_result, file_path, **kwargs):
            ...
            if dtst_cd == "data802":
                select_db_stmt = f'''SELECT c_dan_jo, cn_dansockja, s_no, d_start, d_end, obj_title, obj_addr
                                    , DBMS_LOB.SUBSTR(latitude, DBMS_LOB.GETLENGTH(latitude)) latitude
                                    , DBMS_LOB.SUBSTR(longitude, DBMS_LOB.GETLENGTH(longitude)) longitude'''
            elif  dtst_cd == "data845":
                select_db_stmt = f'''SELECT map_no, map_nm, ins_usr, obj_type
                                    , DBMS_LOB.SUBSTR(obj_val, DBMS_LOB.GETLENGTH(obj_val)) obj_val
                                    , obj_title, obj_time, obj_addr, obj_count, del_yn, ins_dy, del_dy, map_cctv'''
            elif  dtst_cd == "data803":
                select_db_stmt = f'''SELECT 단속번호, 차량구분, 차종, 단속년도, 단속일련번호, 차량번호, 단속일시, 부과여부, 단속동, 단속조, D_START, D_END, "단속조명(지도관리)", "단속주소(지도관리)"
                                    , 단속구분, 단속원금, 납부액, 가산금액, 감액, 미납액, 특별단속지역, 소화전, 대장구분, 단속내용
                                    , DBMS_LOB.SUBSTR(위도, DBMS_LOB.GETLENGTH(위도)) 위도
                                    , DBMS_LOB.SUBSTR(경도, DBMS_LOB.GETLENGTH(경도)) 경도'''

...
class EhojoUtil:
    ...
    def set_url(dtst_cd, params_dict, repeat_num, page_index, interface_id, encrypt_key_ehojo):
        """
        dtst_cd별 url 설정
        ...
        if params_dict != {}:  # 사업, 세부사업, 부서코드, 부서코드이력 제외
            params = params_dict.get("params")
            # data_body["fyr"] = params

            #-------------------- 초기구축 --------------------
            param_list = params_dict.get("param_list")
            data_body["fyr"] = str(param_list[repeat_num - 1])
            #-------------------------------------------------
        ...
        logging.info(f"요청 전문::: {json_data}")  # test
        return json_data

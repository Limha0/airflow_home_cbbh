...
def csv_to_dw():
                ...
                if tn_data_bsc_info.link_file_crt_yn.lower() == 'y' or dtst_cd == 'data917' or\
                    ...
                    with session.begin() as conn:
                        for dict_row in conn.execute(select_log_info_stmt).all():
                            th_data_clct_mastr_log_update = ThDataClctMastrLog(**dict_row)
                            tn_clct_file_info_update = get_file_info(th_data_clct_mastr_log_update.clct_log_sn, conn)
                            CommonUtil.update_log_table(log_full_file_path, tn_clct_file_info_update, session, th_data_clct_mastr_log_update, CONST.STEP_FILE_STRGE_SEND, CONST.STTS_WORK, CONST.MSG_FILE_STRGE_SEND_WORK_UNZIP, "n")
                else:
                    # 로그 업데이트
    ...
    @task
    def move_file_in_storage(**kwargs):
        """
        스토리지로 전송
        """
        import shutil
        ...
        with session.begin() as conn:
            ...
            for dict_row in conn.execute(select_log_info_stmt).all():
                ...
                before_file_path = kwargs['var']['value'].root_final_file_path + file_path  # local test
                # before_file_path = kwargs['var']['value'].final_file_path + file_path
                storage_file_path = kwargs['var']['value'].storage_file_path + file_path
                link_file_extn = tn_data_bsc_info.link_file_extn
                pvdr_sou_data_pvsn_stle = tn_data_bsc_info.pvdr_sou_data_pvsn_stle

                moved_file_path = ""
                os.chdir(before_file_path)
                if not os.path.exists(storage_file_path):  # 경로 없을 시 경로 생성
                    os.makedirs(storage_file_path, exist_ok=True)

                try:
                    for file in os.listdir(before_file_path):
                        if (tn_data_bsc_info.dtst_cd != 'data891' and file.endswith(link_file_extn) and not file.endswith('_sample.' + link_file_extn) and not file.endswith(tn_data_bsc_info.pvdr_site_nm + ".zip") and not file.endswith("enc.zip")):
                            moved_file_path = shutil.copy(before_file_path + file, storage_file_path)
                            logging.info(f"move_file_in_storage moved_file_path::: {moved_file_path}")
                        elif (tn_data_bsc_info.dtst_cd == 'data891' and not file.endswith('zip')):  # 노선별OD데이터
                            if not os.path.exists(storage_file_path + file):  # 경로 없을 시 경로 생성
                                os.makedirs(storage_file_path + file, exist_ok=True)
                            copy_tree(before_file_path + file, storage_file_path + file)
                    ...
                    logging.error(f"move_file_in_storage Exception::: {e}")
        if dict_row == None:
            logging.info(f"move_file_in_storage ::: 스토리지전송 대상 조회 대상없음")
    ...
    def get_bsc_info(dtst_cd, clct_data_nm, clct_log_sn, conn):
        ...
        if dtst_cd == "data675":  # 신문고
            select_bsc_info_stmt = f'''
                SELECT sn, a.dtst_cd, b.dtst_nm, dtst_dc, a.pvdr_site_cd, pvdr_inst_cd, pvdr_dept_nm, pvdr_pvsn_mthd_cd, pvdr_data_se_vl_one, pvdr_data_se_value_two
    ...
    def select_loading_data_list_info(**kwargs):
        ...
        select_log_info_stmt = f"""
                                ...
                                UNION ALL
                                SELECT
                                    b.clct_log_sn, b.dtst_cd, b.clct_ymd, a.dtst_nm, a.pbadms_fld_lclsf_nm, b.data_crtr_pnttm, b.reclect_flfmt_nmtm, b.step_se_cd, b.stts_cd,
    ...
    ext_data_list >> unzip_decrypt_file(ext_data_list) >> check_file_in_final_path() >> move_file_in_storage() >> loading_data_lists >> create_temp_file_in_db(loading_data_lists) >> lists_column_info >> dw_loading(lists_column_info)

dag_object = csv_to_dw()

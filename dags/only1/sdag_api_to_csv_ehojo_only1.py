...
@dag(
    dag_id="sdag_api_to_csv_ehojo_only1",
    schedule="@daily",
    start_date=datetime(2023, 9, 16, tz="Asia/Seoul"),  # UI 에 KST 시간으로 표출하기 위한 tz 설정
    catchup=False,
    # render Jinja template as native Python object
    render_template_as_native_obj=True,
    tags=["api_to_csv_only1", "month", "int"],
)
def sdag_api_to_csv_ehojo_only1():
    ...
    def insert_collect_data_info(**kwargs):
        ...
        select_bsc_info_stmt = '''
                            ...
                            WHERE 1=1
                                ...
                                and dtst_cd in ('data692','data693') -- test 세입예산결산내역, 세출예산결산내역 
                                -- and dtst_cd in ('data873') -- test 세입목코드
                            ORDER BY sn
                            '''
        # data_interval_start = kwargs['data_interval_start'].in_timezone("Asia/Seoul")  # 처리 데이터의 시작 날짜 (데이터 기준 시점)
        data_interval_start = datetime(2020, 1, 1, tz="Asia/Seoul")  # 2020년으로 고정
        # data_interval_end = kwargs['data_interval_end'].in_timezone("Asia/Seoul")  # 실제 실행하는 날짜를 KST 로 설정
        data_interval_end = datetime(2020, 12, 31, tz="Asia/Seoul")  # 2020년으로 고정
        collect_data_list = []
        try:
            with session.begin() as conn:
                for dict_row in conn.execute(select_bsc_info_stmt).all():
                    ...
                    file_name = tn_data_bsc_info.dtst_nm.replace(" ", "_") + "_2020_only"
    ...
    def call_url_process(collect_data_list):
        ...
        def call_url(collect_data_list, file_path, **kwargs):
            ...
            from util.only1.ehojo_util_only1 import EhojoUtil
            ...
            # 파라미터 및 파라미터 길이 설정
            # params_dict, params_len = CallUrlUtil.set_params(tn_data_bsc_info, session, kwargs)
            ...
            try:
                ...
                while repeat_num <= params_len:
                    ...
                    while page_no <= total_page:
                        ...
                        data = EhojoUtil.set_url(dtst_cd, params_dict, repeat_num, page_no, interface_id, encrypt_key_ehojo)
                        ...
                        if response_code == 200:
                            ...
                            result = CallUrlUtil.read_json(json_data, pvdr_site_cd, pvdr_inst_cd, dtst_cd, tn_data_bsc_info.data_se_col_one) # test
                ...
                # if row_count == 0 and fail_count == 0:
...
dag_object = sdag_api_to_csv_ehojo_only1()
